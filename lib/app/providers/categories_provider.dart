...
import 'package:techfrenetic/app/models/mappers/brands_categories_mapper.dart';
import 'package:techfrenetic/app/models/mappers/services_categories_mapper.dart';
...
class CategoriesProvider extends TechFreneticProvider {
  ...
  Future<List<CategoriesModel>> getSubjects() async {
    try {
      Uri _url = Uri.parse("$baseUrl/api/$locale/v1/subject");
      var response = await http.get(_url);
  ...
  Future<List<CategoriesModel>> getServices() async {
    try {
      Uri _url = Uri.parse("$baseUrl/api/$locale/v1/services-vendors");
      var response = await http.get(_url);

      if (response.statusCode == 200) {
        List<dynamic> jsonResponse = json.jsonDecode(response.body);
        return jsonResponse
            .map((e) => ServicesCategoriesMapper.fromMap(e))
            .toList();
      } else {
        debugPrint('Request failed with status: ${response.statusCode}.');
      }
  ...
  Future<List<CategoriesModel>> getBrands() async {
    try {
      Uri _url = Uri.parse("$baseUrl/api/$locale/v1/brand-vendors");
      var response = await http.get(_url);

      if (response.statusCode == 200) {
        List<dynamic> jsonResponse = json.jsonDecode(response.body);
        return jsonResponse
            .map((e) => BrandsCategoriesMapper.fromMap(e))
            .toList();
      } else {
        debugPrint('Request failed with status: ${response.statusCode}.');
      }
